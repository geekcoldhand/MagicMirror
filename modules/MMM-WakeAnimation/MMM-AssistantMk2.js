...
Module.register("MMM-AssistantMk2", {
	defaults: {
		...
		deviceLocation: {
			// (optional)
			coordinates: {
				// set the latitude and longitude of the device (rf. mygeoposition.com)
				latitude: 51.503364, // -90.0 - +90.0
				longitude: -0.127625 // -180.0 - +180.0
			}
		},
	...
	start: function () {
		...
		console.log("starting MMM-AssistantMk2.js ");
	},
	...
	getDom: function () {
		...
		wrapper.onclick = () => {
			if (wrapper.className == "sleeping") {
				// Send notification to other modules FIRST
				this.sendNotification("ASSISTANT_LISTENING");
	...
	notificationReceived: function (notification, payload, sender) {
		switch (notification) {
			...
			case "ASSISTANT_ACTIVATE":
				console.log("*** 4. ASSISTANT_ACTIVATE received:", payload);
				this.sendNotification("ASSISTANT_LISTENING");
				var profileKey = "";
				var profile = {};
				if (payload.profile in this.config.profiles) {
					console.log(" Using requested profile:", profileKey);
					profileKey = payload.profile;
				} else {
					profileKey = this.config.defaultProfile;
					console.log(" Using default profile:", profileKey);
				}
